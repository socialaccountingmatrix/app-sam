  ...
  private db!: IDBDatabase;
   ...
   async checkConnection(): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
        ...
        resolve(
            data.map(({ samKey, createdAt, modifiedAt, name, description }) => ({
            samKey,
            createdAt,
            modifiedAt,
            name,
     ...
     * @param samData Full SAMData object to add.
     * @returns Promise resolving to the generated unique key for the SAM entry.
     */
    async addSAMToList(samData: SAMData): Promise<string> {
        return new Promise((resolve, reject) => {
        const samKey = this.generateSamKey();
        const transaction = this.db.transaction(this.storeName, 'readwrite');
        const store = transaction.objectStore(this.storeName);
        const request = store.add({ ...samData, samKey });
    ...
    async updateEntireSAM(samKey: string, samData: SAMData): Promise<boolean> {
        ...
        const transaction = this.db.transaction(this.storeName, 'readwrite');
        const store = transaction.objectStore(this.storeName);
        const request = store.put({ ...samData, samKey });
